...
resource "aws_eip" "main" {
  ...
  tags = {
    ...
    Name          = format("%s-eip-%s", local.project_prefix, var.build_suffix)
  }
}
...
resource "aws_nat_gateway" "main" {
  ...
  tags = {
    resource_owner = local.resource_owner
    Name          = format("%s-ngw-%s", local.project_prefix, var.build_suffix)
  }
}
...
module subnet_addrs {
  ...
  version         = "1.0.0"
  base_cidr_block = cidrsubnet(local.eks_cidr,2,index(var.azs,each.key))
  networks        = [
    {
      name     = "eks-internal"
      new_bits = 1
...
resource "aws_route_table" "main" {
  ...
  tags = {
    Name = format("%s-eks-rt-%s", local.project_prefix, var.build_suffix)
  }
}
